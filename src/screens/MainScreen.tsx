...
export const MainScreen: React.FC<{ onOpenAdmin: () => void, onOpenQr: () => void }> = ({ onOpenAdmin, onOpenQr }) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto relative z-10 flex flex-col items-center justify-center p-4 md:p-6 w-full min-h-full animate-fade-in pb-[max(env(safe-area-inset-bottom),1.5rem)]">
      <GlassCard className="w-full max-w-lg p-8 md:p-10 rounded-sm relative shadow-2xl flex flex-col items-center">
        {/* Corner Accents */}
        ...
        {/* Tag */}
        <div className="inline-flex items-center gap-2 px-3 py-1 bg-cyan-950/40 border border-cyan-900/50 rounded-sm mt-8 mb-6">
        ...
        {/* Title Area */}
        <h1 className="text-3xl md:text-4xl font-kor font-black text-white tracking-tight mb-2 text-center drop-shadow-md">
          CBT_TOPIK
        </h1>
        <h2 className="text-sm md:text-base font-tech font-bold text-[#00b050] tracking-[0.1em] mb-12 text-center drop-shadow-[0_0_8px_rgba(0,176,80,0.4)] uppercase">
